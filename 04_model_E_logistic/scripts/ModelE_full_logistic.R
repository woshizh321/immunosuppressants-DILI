...
out_csv_pat <- "04_model_E_logistic/outputs/model/model_pattern_counts.csv"
dir.create(dirname(out_csv_pat), recursive = TRUE, showWarnings = FALSE)

# =========================
# 1) DuckDB connect + QC schema
# =========================
...
# =========================
# 2) case_demo / case_dili / psss_is_rows
# =========================
...
# =========================
# 3) Build 5 exposure flags (TEMP tables)
# =========================
...
# =========================
# 4) Assemble model_case_table (关键：你缺的就是这张表)
# =========================
...
# =========================
# 5) Build model_pattern_counts + export
# =========================
...
write.csv(pat, out_csv_pat, row.names = FALSE)
cat("Saved pattern table to:\n", out_csv_pat, "\n")
# ============================================================
...
# =========================
# 1) Load model & pattern table
# =========================
fit <- readRDS("04_model_E_logistic/outputs/model/modelE_logistic_fit.rds")
pat <- read.csv("04_model_E_logistic/outputs/model/model_pattern_counts.csv")

# QC
stopifnot(all(c("n","n_dili") %in% colnames(pat)))
stopifnot(sum(pat$n_dili) <= sum(pat$n))

# =========================
# 2) Predict probability (pattern-level)
# =========================
pat$pred_prob <- predict(fit, newdata = pat, type = "response")

# =========================
# 3) Expand to case-level (weighted)
# =========================
y_true <- c(
  rep(1, sum(pat$n_dili)),
  rep(0, sum(pat$n - pat$n_dili))
)

y_score <- c(
  rep(pat$pred_prob, pat$n_dili),
  rep(pat$pred_prob, pat$n - pat$n_dili)
)
...
# 5) Summary
# =========================
modelE_metrics <- data.frame(
  model = "Model_E (Logistic, refit)",
  AUROC = auroc,
  AUPRC = auprc,
  Brier = brier
)

print(modelE_metrics)

write.csv(
  modelE_metrics,
  "04_model_E_logistic/outputs/model/Model_E_metrics.csv",
  row.names = FALSE
)
